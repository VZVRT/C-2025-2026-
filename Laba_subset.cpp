        ...
        if (k < (*sn)->key) {
            ...
            if ((*sn)->left == nullptr) {
                ...
                *sn = (*sn)->left;
                ...
                subset_node** min_pr = &(*sn)->right;
                while ((*min_pr)->left != nullptr) {
                    min_pr = &(*min_pr)->left;
                }
                (*sn)->key = (*min_pr)->key;
                subset_node*a = *min_pr;
                *min_pr = (*min_pr)->right;
                delete a;
            }
            return true;
...
unsigned int height(subset_node *sn) {
    ...
    unsigned int a = height(sn->left);
    unsigned int b = height(sn->right);
    if (a > b) {
        return 1 + a;
    }
    else { return 1 + b; }
...
void help(subset_node* sn, int* res, unsigned int* i) {
    if (sn == nullptr) {  
        return;
    }
    help(sn->left, res, i);   
